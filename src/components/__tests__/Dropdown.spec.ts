...
describe('Dropdown.vue', () => {
	...
	it('does not disable the dropdown when the disabled prop is false', () => {
		const wrapper = mount(Dropdown, {
			props: { items, placeholder, disabled: false },
		})

		const select = wrapper.find('select')
		expect(select.element.disabled).toBe(false)
		expect(select.classes()).not.toContain('cursor-not-allowed')
	})
})
